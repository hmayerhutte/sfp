...
import SFPLogger, { COLOR_INFO,COLOR_TRACE,COLOR_ERROR,COLOR_SUCCESS,COLOR_WARNING,COLOR_HEADER, COLOR_KEY_MESSAGE, LoggerLevel } from '@flxblio/sfp-logger';
...
export default class PackageInstall {
  ...
  async run(
    ...
    const packageTrees = project.getPackageDirectories()
    const packageVersionMap = new Map<string, string>()
    SFPLogger.log(COLOR_HEADER(`💪 Put packages in queue and start deployment/installation process`),LoggerLevel.INFO)
    const q = new PQueue({concurrency: 1})
    for (const [packageName, packageCharacter] of packageMap) {
      if (packageCharacter.type !== 'unlocked') {
        SFPLogger.log(
          COLOR_WARNING(
            `Found changes in package ${packageName}. 👆 Skipping validation because no unlocked package ( Found no package id )!`, LoggerLevel.INFO
          ),
        )
        continue
      }
    ...
    Lifecycle.getInstance().on(PackageEvents.install.status, async (results: PackageInstallRequest) => {
        SFPLogger.log(
        COLOR_TRACE(
          `⌛ Install package status: ${results.Status} SubscriberId: ${results.SubscriberPackageVersionKey}`, LoggerLevel.INFO
        ),
      )
    })

    await q.onIdle()
    SFPLogger.log(COLOR_HEADER(`Yippiee. 🤙 Validation finsihed without errors. Great 🤜🤛`), LoggerLevel.INFO)
  }
  ...
  private async installPackage(
    ...
    if (versionNumber) {
      ...
      if (this.runScripts && preDeploymentScriptPath && isKey) {
        SFPLogger.log(COLOR_INFO(`☝ Found pre deployment script for dependency package ${packageName}`), LoggerLevel.INFO)
      ...
      if (
        ...
        if (targetTreePck?.versionNumber) {
          ...
          if (package2Version) {
            ...
            SFPLogger.log(COLOR_TRACE(`☝ Found update for child package ${packageName}`), LoggerLevel.INFO)
            SFPLogger.log(
              COLOR_TRACE(
                `So first install the old version ${targetTreePck.versionNumber} from main branch and then deploy the package`, LoggerLevel.INFO
              ),
            )
          }
        }
      ...
      if (this.runScripts && postDeploymentScriptPath && isKey) {
        SFPLogger.log(COLOR_INFO(`☝ Found post deployment script for dependency package ${packageName}`))
  ...
  private printInstallationHeader(packageName: string, packageCharacter: PackageCharacter): void {
    SFPLogger.log(COLOR_INFO(`${COLOR_HEADER('Package:')} ${packageName}`))
    if (packageCharacter.packageDeps.length > 0) {
      ...
      SFPLogger.log(`${COLOR_HEADER('Dependencies:')} ${COLOR_INFO(depsList.join(','))}`)
    }
  }

  private async handlePackageError(packageName: string, errorMessage: string): Promise<void> {
    SFPLogger.log(COLOR_ERROR(`💥 Package ${packageName} runs on error!`),LoggerLevel.INFO)
    SFPLogger.log(COLOR_ERROR(errorMessage), LoggerLevel.INFO)
    exit(1)
  }
  ...
  private async deployPackageFromPath(pck: string, path: string, orgAlias: string): Promise<void> {
    ...
    deploy.onUpdate((response) => {
      if (counter === 5) {
        ...
        SFPLogger.log(COLOR_TRACE(message), LoggerLevel.INFO)
        counter = 0
      } else {
        counter++
      }
    ...
    if (res?.response?.success) {
        SFPLogger.log(COLOR_INFO(`✔ Deployment for Package ${pck} successfully 👌`), LoggerLevel.INFO)
    } else {
  ...
  private async installPackageWithId(
    ...
    SFPLogger.log(
      COLOR_TRACE(`⌛ Install Package: ${packageName} SubscriberId: ${package2Version.SubscriberPackageVersionId}`), LoggerLevel.INFO
    )

    const installResult = await subscriberPackageVersion.install(packageInstallRequest, options)
    if (installResult.Status === 'SUCCESS') {
        SFPLogger.log(COLOR_INFO(`✔ Installation for Package ${packageName} successfully 👌`))
      return
    }
    ...
    if (
      ...
      SFPLogger.log(COLOR_ERROR(`Installation errors:`))
      let errorCounter = 1
      // eslint-disable-next-line no-unsafe-optional-chaining
      for (const error of installResult.Errors?.errors) {
        SFPLogger.log(COLOR_ERROR(`${errorCounter}) ${error.message}`))
        errorCounter++
      }
  ...
  private async runDeploymentSteps(scriptPath: string, scriptStep: string, packageName: string, alias: string) {
    SFPLogger.log(COLOR_TRACE(`Execute deployment script`))
    SFPLogger.log(`${COLOR_TRACE('Path:')} ${COLOR_TRACE(scriptPath)}`)
    const stateAggregator = await StateAggregator.getInstance()
    ...
    try {
      ...
      if (stderr) {
        SFPLogger.log(COLOR_ERROR(`${scriptStep} Command Error: ${stderr}`))
      }

      if (stdout) {
        SFPLogger.log(COLOR_INFO(`${scriptStep} Command Info: ${stdout}`))
      }
    } catch (error) {
        SFPLogger.log(COLOR_ERROR(`${scriptStep} Command Error: ${error}`))
    }
  }

  private async getApexClassesFromPaths(pck: string, path: string, orgAlias: string): Promise<void> {
    SFPLogger.log(COLOR_HEADER(`💪 Start Apex tests for package ${pck}.`))
    ...
    SFPLogger.log(`${COLOR_KEY_MESSAGE('Package:')} ${COLOR_INFO(pck)}`)
    SFPLogger.log(
      `${COLOR_KEY_MESSAGE('Testclasses:')} ${COLOR_INFO(
    ...
    SFPLogger.log(COLOR_INFO(`⌛ Waiting For test run results`))
    let _i: number = 2
  ...
  private async checkTestResult(apexClassList: string[], jobId: string[], orgAlias: string): Promise<void> {
    ...
    try {
      ...
      if (responseFromOrg.records.length > 0) {
        ...
        console.log(table.toString())
        SFPLogger.log(COLOR_ERROR(`This package contains testclass errors.`))
  ...
  private async checkCodeCoverage(ids: string[], orgAlias: string): Promise<void> {
    ...
    if (coveredCounter === 0 && uncoveredCounter === 0) {
      SFPLogger.log(COLOR_WARNING(`This package has no covered and uncovered lines. Please check the result. 👆`))
    } else {
      packageCoverage = Math.floor((coveredCounter / (coveredCounter + uncoveredCounter)) * 100)

      SFPLogger.log(COLOR_INFO('Check Code Coverage for Testclasses:'))
      SFPLogger.log(COLOR_INFO(table.toString()))
      if (packageCoverage < 75) {
        ...
        SFPLogger.log(COLOR_SUCCESS(`👏 Great. This package has a code coverage from ${packageCoverage}%. 😊`))
      }
    }
  }
}
