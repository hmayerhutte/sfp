...
import ImpactedPackageResolver from '../impact/ImpactedPackagesResolver';
...
export default class ValidateImpl implements PostDeployHook, PreDeployHook {
    ...
    private impactedPackagesAsPerBranch: Map<string, string[]>;
    ...
    public async exec(): Promise<ValidateResult> {
        ...
        try {
            if (this.props.validateAgainst === ValidateAgainst.PROVIDED_ORG) {
                ...
                else targetUserName = await this.fetchScratchOrgFromPool(this.props.pools, this.props.orgInfo);
            ...
            if (this.impactedPackagesAsPerBranch) {
                SFPLogger.log(
                    COLOR_KEY_MESSAGE(`Packages impacted in ${this.props.branch} vs ${this.props.baseBranch}`),
                    LoggerLevel.INFO,
                    new ConsoleLogger()
                );
    ...
    private async computePackagesChangedAgainstBaseBranch() {
    ...
    private async printArtifactVersions(orgAsSFPOrg: SFPOrg, installedArtifacts: any) {
    ...
    private async deployPackages(scratchOrgUsername: string): Promise<DeploymentResult> {
        ...
        function printDeploySummary(deploymentResult: DeploymentResult, totalElapsedTime: number): void {
            ...
            if (deploymentResult.failed.length > 0) {
                ...
                FileOutputHandler.getInstance().appendOutput(
                    `validation-error.md`,
                    `Package validation failed for  **${firstPackageFailedToValdiate.sfpPackage.packageName}** due to`
                );
                ...
                deploymentResult.failed.map((packageInfo, index) => {
                    if (index != 0)
                        FileOutputHandler.getInstance().appendOutput(
                            `validation-error.md`,
                            `**${packageInfo.sfpPackage.packageName}**`
                        );
                });
            }
    ...
    private async buildImpactedPackages(packagesInstalledInOrgMappedToCommits: {
        [p: string]: string;
    }): Promise<SfpPackage[]> {
        let groupSection = new GroupConsoleLogs('Building packages for Synchronization+Validation').begin();
        ...
        if (failedPackages.length > 0) throw new Error(`Failed to create packages ${failedPackages}`);
        ...
        function fetchPackagesAsPerReleaseConfig(logger: Logger, props: ValidateProps) {
            if (
                ...
                function printIncludeOnlyPackages(includeOnlyPackages: string[]) {
                    SFPLogger.log(
                        COLOR_KEY_MESSAGE(
                            `Validate will include the below packages release configs (domain(s))(domain)`
                        ),
                        LoggerLevel.INFO
                    );
    ...
    async preDeployPackage(
        ...
        const shouldInstallDependencies = (mode: ValidationMode) => {
            if (
                this.props.validateAgainst === ValidateAgainst.PROVIDED_ORG &&
                !this.props.installExternalDependencies
            ) {
                return false;
            }

            const isThoroughValidation =
                mode === ValidationMode.THOROUGH || mode === ValidationMode.THOROUGH_LIMITED_BY_RELEASE_CONFIG;
        ...
        if (isPackageImpacted) {
            if (sfpPackage.packageType && sfpPackage.packageType != PackageType.Data) {
                if (packageInstallationResult.result === PackageInstallationStatus.Succeeded) {
                    ...
                    const apextestValidator = new ApexTestValidator(
                        targetUsername,
                        sfpPackage,
                        this.props,
                        this.logger
                    ...
                    if (!testResult.result) {
                        ...
                        FileOutputHandler.getInstance().appendOutput(
                            `validation-error.md`,
                            `Package validation failed for  **${sfpPackage.packageName}**`
                        );
