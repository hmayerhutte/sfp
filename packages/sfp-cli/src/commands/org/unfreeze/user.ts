...
const messages = Messages.loadMessages('@flxbl-io/sfp', 'org_unfreeze_user');
...
export default class OrgUnfreezeUser extends SfpCommand {
    ...
    public async execute(): Promise<any> {
        ...
        let profilesList: string[] = [];
            ...
            SFPLogger.log(COLOR_KEY_MESSAGE(`Unfreeze users for this profiles 👇`));
        ...
        query = `Select Id,UserId, IsFrozen from UserLogin Where UserId In ('${Array.from(userSet).join(`','`)}') And IsFrozen = true`;
        ...
        SFPLogger.log(COLOR_KEY_MESSAGE(`Found this users to unfreeze 👇`));
        ...
        SFPLogger.log(COLOR_TRACE(`Start unfreezing users in the org ...`));
        ...
        for (let user of userLoginList) {
            await limiter.schedule(async () => {
                try {
                    const updateResult = await conn.sobject('UserLogin').update({ Id: user.Id, IsFrozen: false });
        ...
        if (succesfullyFreezedCount > 0) {
            SFPLogger.log(COLOR_SUCCESS(`✅ Succesfully unfreezed: ${succesfullyFreezedCount}`));
        }
        if (failedFreezedCount > 0) {
            SFPLogger.log(COLOR_ERROR(`❌ Failed to unfreeze: ${failedFreezedCount}`));
        }
